...
int find_empty(struct PTE page_table[TABLEMAX], int *table_cnt){
    for (int i = 0; i < *table_cnt; i++){
        struct PTE current = page_table[i];
        if (current.is_valid == 0){
            return i;
        }
    }
...
int process_page_access_lru(struct PTE page_table[TABLEMAX], int *table_cnt, int page_number, int frame_pool[POOLMAX], int *frame_cnt, int current_timestamp) { return -1; };
...
int process_page_access_lfu(struct PTE page_table[TABLEMAX], int *table_cnt, int page_number, int frame_pool[POOLMAX], int *frame_cnt, int current_timestamp) { return -1; };
